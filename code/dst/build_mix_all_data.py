...
if not os.path.exists('data/mix_all_dst_{}'.format(target_lang)):
    os.makedirs('data/mix_all_dst_{}'.format(target_lang))

# save train dials into mix_dst_english
with open('data/mix_all_dst_{}/train_dials.json'.format(target_lang), 'w', encoding='utf-8') as f:
    json.dump(train_dials, f, indent=2, ensure_ascii=False)
with open('data/mix_all_dst_{}/dev_dials.json'.format(target_lang), 'w', encoding='utf-8') as f:
    json.dump(dev_dials, f, indent=2, ensure_ascii=False)
with open('data/mix_all_dst_{}/test_dials.json'.format(target_lang), 'w', encoding='utf-8') as f:
